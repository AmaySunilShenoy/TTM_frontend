        ...
        <div className="embla__container">
          {slides.map((slide,index) => (
            <div className="embla__slide text-white flex" key={index}>
              ...
              <p className={`text-[30px] ${helveticaBold.className}`}>{slide.name}</p>
              <p className={`text-[18px] ${helveticaLight.className}`}>{slide.desc}</p>
              </div>
              <div className={`${helveticaBold.className} text-right`}>
              <button className="bg-white text-black w-full py-2 shadow-lg rounded-2xl transition-all hover:-translate-y-2 hover:text-white hover:bg-slate-900">Chat</button>
              </div>

              </div>
              <img className="w-[525px]" src={`/assets/${slide.image}`} alt="" />
            </div>
          ))}
        </div>
      </div>
